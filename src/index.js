...
export function styleTrans(style) {
  ...
  Object.keys(style).forEach(cssName => {
    ...
    if (cssName === "tranform" && cssValue instanceof Array) {
      cssValue.forEach(item => {
        if (typeof item.translateX === "number")
          item.translateX = pxTrans(item.translateX);
        if (typeof item.translateY === "number")
          item.translateY = pxTrans(item.translateY);
      });
    }
    if (
      reg1.test(cssName) &&
    ...
    if (reg2.test(cssName)) {
      if (typeof style[cssName].height === "number")
        style[cssName].height = pxTrans(style[cssName].height);
      if (typeof style[cssName].width === "number")
        style[cssName].width = pxTrans(style[cssName].width);
    }
  });
  return style;
}
...
export function create(obj) {
  return StyleSheet.create(styleTransAll(obj));
}
